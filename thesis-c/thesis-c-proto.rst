...
.. The goals of this project:
   ...
   * Scope: fixed-priority hard real-time
   * Time-demand analysis
   * What does the kernel need to guarantee for time-demand analysis?
      - Bounds on external executions
      - Sufficient budget for internal executions
   * How does the kernel provide these guarantees?
      - Bounded task execution
      - Bounded priority inversion
      - IRQ rate limiting & budgeting
      - Preemption budget donation
   * What changes need to be made to the kernel?
      - IRQ SCs
      - Bounded time donation for ICPC
      - Bandwidth donation on preemption
   * How can we show that the kernel satisfies these guarantees?
   * Using the time-demand model to generate test workloads
   * Demonstrating correct scheduling of test workloads
   * What are the limits on utilisation?

   * Rectify a number of issues preventing real-time analysis of MCS
     systems
   * Bounded priority inversion
   * Bandwidth donation on IPC
   * Bounded / accounted interrupt delivery
   * Faulting semantics?
   * Response time analysis / time-demand analysis
     - Fixed-priority
     - Priority ceiling
     - For each task t in tasks
         assert(t.relative_deadline <= t.period) # Maybe account for more general case?
         time_demand = t.wcet + t.maximum inversion
         time_demand += sum([v.wcet for v in tasks if v.prio >= t.prio])
         assert(time_demand <= t.relative_deadline)
         # For sporadic, each replenshment counts as a task?
         # Do we need to consider each subtask of a sporadic task separately?
         # Do we need to consider each task at an equal priority level? separately (i don't think so)
         # How do constant bandwidth tasks contribute to demand function?
...
.. note:: I'm unsure where to arrange the following sections. I need to
   take a look at some older theses to see what the final structure
   should be.
...
.. note::
   ...
   Q: What do we depend on?
   A: Maximum execution time between release and deadline

   Q: What contributes to that execution?
   A: Job execution, higher priority jobs, inverted priority execution,
      system execution

   Q: How must that be bounded?
   A: Predictable maximum of any contributing execution in all possible
      windows of relative deadline size.

For hard realtime tasks, we must guarantee that each *job* in the task
is able to complete before its *deadline*. For a job to complete before
its deadline, not only must complete its own execution entirely before
the deadline, but all higher-priority work the needed to be done after
the jobs release must also be executed before the jobs own work.

The *time demand* for some job :math:`T_j` is the total amount of
execution time required between the release of :math:`T_j` and the end
of the execution of :math:`T_j`. In addition to execution time of
:math:`T_j`, this includes the remaining execution of any
higher-priority jobs that were released prior to the release of
:math:`T_j` as well as the execution of any higher priority jobs
released between the release of :math:`T_j` and the end of execution of
:math:`T_j`. This also includes any work performed on behalf of lower
priority jobs that was scheduled at a priority above :math:`T_j`, such
as accesses to shared resources.

The worst case of time demand for a task :math:`T` is the largest
possible time demand of a job in :math:`T` given all possible
schedules. In a system of fixed-priority tasks, the worst case time
demand would occur when a job from each task at an equal or higher
priority is released at the same time as a job in :math:`T` in
conjunction with the start of a priority inversion for some
lower-priority job. If we assume a minimum inter-arrival time, a
constant relative deadline, and a worst case execution time for each
task as well as a worst-case priority inversion time for the priority
level of :math:`T`, then we can determine the time demand for the
worst-case situation.

The time demand for the worst case will be the sum of:

 * the worst case cost for priority inversion,
 * the sum of the worst-case execution costs for each of the equal or
   higher priority tasks multiplied by the maximum number of arrivals
   that could occur within the relative deadline of the task, and
 * the worst case cost of the task itself.

If this cost is less than the relative deadline of the task, then the
task is guaranteed to always complete execution before its deadline
given the aforementioned assumptions. As such, all jobs in a task set
may be guaranteed to meet their deadlines if the above is true for all
tasks.

In a deadline-monotonic system, where tasks at an equal priority are
scheduled with an equal realtive deadline, if one task at a given
priority level is guaranteed to meet deadlines, then all tasks at that
priority will be guaranteed to meet deadlines.

.. note::

   We actually only require that the total time required to execute
   higher-priority tasks within the relative deadline remains bounded
   at or below some maximum. The actual number and frequency of tasks
   could change in ways that do not affect this assumption.

   Sporadic server allows a tasks execution time of a task or job to be
   subdivided and delayed in a manner that doesn't increase the amount
   of execution time needed for a given priority level in any particular
   time window.

   We also assume that tasks will execute to completion when there are
   no higher priority tasks executing.

   We don't actually need to know a minimum inter-arrival time and
   worst-case execution, we need to know some window size for a task and
   the maximum amount of time a task will deman within that window size.

   (A sliding window on time demanded rather than on execution time).

Kernel guarantees for time-demand analysis
------------------------------------------

In the previous section, we outlined a number of assumptions that are
assumed to be true for the scheduling analysis to hold:

 1) each task has a known budgeting window,
 2) each task has a maximum demanded budget per window,
 3) each task has a minimum relative deadline, and
 4) each priority level has a known worst-case delay due to priority
    inversion.

For our scheduling analysis to guarantee that all deadlines will be met,
we must ensure that these assumptions are all actually true. (1), (2),
and (4) assume a certain behavior of tasks other than the one for which
the guarantee will be provided with respect to the deadline from (3).

To ensure the other tasks actually behave as assumed, we can blindly
trust that they do, mathematically verify that they do, or construct our
scheduler such that they can never be scheduled such that they violate
these assumptions. The MCS extensions to seL4 use the latter approach,
requiring only verification of the scheduler to ensure the requirements
of a real-time system are met under all circumstances.

The kernel scheduler must ensure that:

 * if there is a task with unconsumed budget, then a task with available
   budget and a priority equal to the highest priority of all tasks with
   unconsumed budget is executing and consuming budget equal to time
   spent executing,
 * within the budgeting window of a task, that task never demands more
   than it's maximum demanded budget per window,
 * no task below a given priority level may execute above that priority
   level for longer than that priority level's maximum delay due to
   priority inversion, and
 * all non-idle execution time is attributed to some task.
